...
const OPTIONAL_PANEL_STORAGE_KEY = 'edge15.optionalPanels.v1.clean';
const DEFAULT_OPTIONAL_PANELS = {
  trackerStatus: false,
  commitmentAccuracy: false,
  backupCompare: false,
  entryValue: false,
  commitTimingLab: false,
  adaptiveCommitLab: false,
  strategyProfileLab: false,
  focusedDashboard: false,
  positionControls: false,
  tradeReplay: false,
};

type OptionalPanelKey = keyof typeof DEFAULT_OPTIONAL_PANELS;
...
type PreCommitPreview = {
  projectedPick: 'OVER' | 'UNDER' | 'NO TRADE';
  wouldLockNow: 'YES' | 'NO';
  strength: number;
...
type PositionExitRead = {
  status: 'HOLD' | 'WATCH CLOSELY' | 'DANGER' | 'CASH OUT SIGNAL';
  holdQuality: 'STRONG' | 'OK' | 'WEAK' | 'BROKEN';
  score: number;
  profitProtection: string;
  salvageWarning: string;
  reasons: string[];
  tone: Tone;
};
...
export function GenesisDashboard() {
  ...
  const [optionalPanels, setOptionalPanels] = useState(DEFAULT_OPTIONAL_PANELS);
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem(OPTIONAL_PANEL_STORAGE_KEY);
    if (!saved) return;
    try {
      setOptionalPanels({ ...DEFAULT_OPTIONAL_PANELS, ...JSON.parse(saved) });
    } catch {
      window.localStorage.removeItem(OPTIONAL_PANEL_STORAGE_KEY);
    }
  }, []);

  function toggleOptionalPanel(key: OptionalPanelKey) {
    setOptionalPanels((previous) => {
      const next = { ...previous, [key]: !previous[key] };
      window.localStorage.setItem(OPTIONAL_PANEL_STORAGE_KEY, JSON.stringify(next));
      return next;
    });
  }
  ...
  function exportPerformanceData() {
    ...
    link.href = url;
    link.download = `edge15-genesis-027-performance-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const preCommitPreview = useMemo(() => buildPreCommitPreview(decision, activeSignal, countdown, tradeQuality, entryValue, flipRisk), [decision, activeSignal, countdown, tradeQuality, entryValue, flipRisk]);
  const positionExitRead = useMemo(() => position && positionAssessment ? buildPositionExitRead(position, positionAssessment, decision, countdown, tradeQuality, entryValue, flipRisk) : null, [position, positionAssessment, decision, countdown, tradeQuality, entryValue, flipRisk]);
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-4 px-4 py-5 sm:px-6">
      <header className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs font-bold uppercase tracking-[0.38em] text-edge-blue">Genesis-027</div>
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="hidden rounded-full border border-edge-line bg-black/20 px-3 py-1 text-[11px] text-edge-muted sm:block">4m commit • pre-commit preview • exit guard</div>
        </div>
      </header>
          ...
          <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <div className="space-y-3">
              ...
              <div className={`rounded-3xl border px-5 py-6 ${badgeClass(activeSignal?.tone ?? decision.tone)}`}>
                ...
                <PreCommitPreviewCard preview={preCommitPreview} />
              </div>
            </div>
          </div>
        </Panel>
      ...
      <section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <Panel title="Clean cockpit">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Metric label="Projected Pick" value={preCommitPreview.projectedPick} detail={preCommitPreview.wouldLockNow === 'YES' ? 'Lock-ready' : 'Stable preview'} help={preCommitPreview.message} tone={preCommitPreview.tone} />
            ...
            <Metric label="Entry Value" value={entryValue.label} detail={entryValue.askCents === null ? 'Model-only' : `${entryValue.askCents}¢ ask`} help={entryValue.message} tone={entryValue.tone} />
            <Metric label="Flip Risk" value={flipRisk.level} detail={`${flipRisk.flips} recent flips`} help={flipRisk.message} tone={flipRisk.tone} />
          </div>
          <div className="mt-4 rounded-2xl border border-edge-blue/30 bg-edge-blue/10 p-4 text-sm leading-6 text-slate-200">
            <span className="font-black text-edge-blue">Stable preview mode:</span> Edge15 only shows a playable projected side when quality, value, stability, and flip-risk agree. Otherwise it shows NO TRADE instead of bouncing every refresh.
          </div>
        </Panel>
      ...
      {position && positionAssessment ? (
        <Panel title="Position management">
          ...
          {positionExitRead ? <PositionExitPanel read={positionExitRead} /> : null}
      ...
      {optionalPanels.positionControls ? (
        <Panel title="Position controls">
          ...
          <div className="mt-3 text-xs leading-5 text-edge-muted">These buttons are hidden by default so the main cockpit stays clean. Use them only after you actually enter a position and want Edge15 to manage the exit.</div>
        </Panel>
      ) : null}

      <Panel title="Show / hide panels">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
          {Object.entries(optionalPanels).map(([key, visible]) => (
            <button key={key} onClick={() => toggleOptionalPanel(key as OptionalPanelKey)} className={`rounded-xl border px-3 py-2 text-xs font-bold ${visible ? 'border-edge-blue/50 bg-edge-blue/10 text-edge-blue' : 'border-edge-line bg-black/20 text-edge-muted'}`}>
              {visible ? 'Hide' : 'Show'} {optionalPanelLabel(key as OptionalPanelKey)}
            </button>
          ))}
        </div>
      </Panel>

      {optionalPanels.trackerStatus ? <TrackerStatusPanel status={trackerStatus} recheckStatus={recheckStatus} onRecheck={recheckPendingResults} /> : null}
      {optionalPanels.commitmentAccuracy ? <CommitmentAccuracyPanel records={commitmentAccuracy} activeSignal={activeSignal} /> : null}
      {optionalPanels.backupCompare ? <PerformanceBackupPanel records={commitmentAccuracy} status={backupStatus} onExport={exportPerformanceData} onCopy={backupToClipboard} onRestore={triggerRestore} /> : null}
      {optionalPanels.entryValue ? (
      ...
      {optionalPanels.adaptiveCommitLab ? <AdaptiveCommitLabPanel records={adaptiveCommitLab} currentRead={adaptiveCommitRead} countdown={countdown} /> : null}
      {optionalPanels.commitTimingLab ? <CommitTimingLabPanel records={commitTimingLab} countdown={countdown} /> : null}
      {optionalPanels.strategyProfileLab ? <StrategyProfileLabPanel records={strategyProfileLab} /> : null}
      {optionalPanels.tradeReplay ? <TradeReplayPanel records={commitmentAccuracy} /> : null}
      {optionalPanels.focusedDashboard ? (
        <Panel title="Decision details">
      ...
      {visibleSections.genesisStatus ? (
        <Panel title="Genesis-027 status">
          <ul className="list-disc space-y-2 pl-5 text-sm text-edge-muted">
            ...
            <li>Genesis-027 keeps the 4:00 live commitment test from Genesis-026 but cleans up the cockpit.</li>
            <li>Genesis-027 preserves the guardrails, keeps labs running in the background, and adds hide/show controls for the noisy panels.</li>
          </ul>
        </Panel>
      ) : null}
    </main>
  ...
  return (
    <Panel title="Strategy Profile Lab">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Metric label="Live Logic" value="4m Commit" detail="Timing-lab winner" help="Genesis-027 tests live commitment at 4:00 remaining while keeping 8:00, 6:00, 3:00, strategy, and adaptive tests in the labs." tone="blue" />
      </div>
      <div className="mt-4 overflow-hidden rounded-2xl border border-edge-line">
      ...
      <div className="mt-3 rounded-2xl border border-edge-line bg-black/20 p-3 text-xs leading-5 text-edge-muted">
        Captures remain a lab read. Later, we can combine this with Commit Timing Lab data to pick the best timing and the best profile together.
      </div>
    </Panel>
  );
}
  ...
  return (
    <Panel title="Backup + compare">
      <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3">
        <Metric label="Current version" value="Genesis-027" detail="adaptive lab" help="Genesis-027 uses a 4:00 live commit, keeps the adaptive shadow lab, and adds pre-commit preview plus position exit management." tone="blue" />
...
function TradeQualityPanel({ quality, autoTightening, flipRisk }: { quality: TradeQuality; autoTightening: AutoTighteningProfile; flipRisk: FlipRisk }) {
  return (
    <Panel title="Trade quality">
      ...
      <div className="mt-3 rounded-2xl border border-edge-line bg-black/20 p-3 text-xs leading-5 text-edge-muted">
        Genesis-027 keeps this score available, but hides the larger panel by default so the cockpit stays clean.
      </div>
    </Panel>
  );
}
...
function TradeReplayPanel({ records }: { records: CommitmentAccuracyRecord[] }) {
  ...
  return (
    <Panel title="Trade replay snapshots">
      ...
      <div className="mt-3 text-xs leading-5 text-edge-muted">Replay records help identify bad setups without manual buttons. Older records may not have every Genesis-027 snapshot field until new commitments are created.</div>
    </Panel>
  );
}
...
function buildTradeQuality(decision: Decision, activeSignal: SignalPlan | null, countdown: Countdown, snapshot: MarketSnapshot, autoTightening: AutoTighteningProfile, flipRisk: FlipRisk): TradeQuality {
  ...
  const payoutPenalty = oddsAsk === null ? (countdown.remainingMs <= 180000 ? 10 : 3) : oddsAsk >= 92 ? 24 : oddsAsk >= 88 ? 14 : oddsAsk >= 82 ? 6 : 0;
...
function buildPreCommitPreview(decision: Decision, activeSignal: SignalPlan | null, countdown: Countdown, tradeQuality: TradeQuality, entryValue: EntryValue, flipRisk: FlipRisk): PreCommitPreview {
  if (activeSignal?.commitmentStatus === 'COMMITTED') {
    return {
      projectedPick: activeSignal.committedDirection === 'OVER' || activeSignal.committedDirection === 'UNDER' ? activeSignal.committedDirection : 'NO TRADE',
      wouldLockNow: 'YES',
      strength: Math.round((activeSignal.committedConfidence ?? decision.confidence) * 0.45 + tradeQuality.score * 0.35 + entryValue.score * 0.2),
      timeUntilLock: 'Locked',
      tone: activeSignal.committedDirection === 'OVER' || activeSignal.committedDirection === 'UNDER' ? 'good' : 'neutral',
      message: `Edge15 already locked ${activeSignal.committedDirection}. The side will not flip this contract; position-management signals handle risk from here.`,
      changes: ['Only risk status can change now', 'Use Hold / Watch / Cash-Out Guard after entry', 'No late side-flipping', 'Next window resets the preview'],
    };
  }
  if (activeSignal?.commitmentStatus === 'NO TRADE') {
    return {
      projectedPick: 'NO TRADE',
      wouldLockNow: 'YES',
      strength: Math.max(0, Math.min(100, Math.round((100 - tradeQuality.score) * 0.45 + (100 - entryValue.score) * 0.25 + 20))),
      timeUntilLock: 'Locked',
      tone: 'neutral',
      message: 'Edge15 already committed NO TRADE for this contract. It will not chase a late prediction.',
      changes: ['Wait for next contract', 'No fresh chase entries', 'Keep watching timing labs', 'Use this as safety data'],
    };
  }

  const secondsToLock = Math.max(0, Math.ceil((11 * 60 * 1000 - countdown.elapsedMs) / 1000));
  const timeUntilLock = secondsToLock <= 0 ? 'Due now' : `${Math.floor(secondsToLock / 60)}:${String(secondsToLock % 60).padStart(2, '0')}`;
  const strength = Math.round(Math.max(0, Math.min(100, decision.confidence * 0.22 + decision.opportunity * 0.2 + decision.stability * 0.22 + tradeQuality.score * 0.24 + entryValue.score * 0.12)));
  const side = activeSignal?.direction && activeSignal.direction !== 'NONE' ? activeSignal.direction : decision.direction;
  const signalStableEnough = (activeSignal?.stability ?? decision.stability) >= 66 && (activeSignal?.confirmations ?? 0) >= 2;
  const planMatureEnough = activeSignal?.status === 'READY' || activeSignal?.status === 'ENTER' || activeSignal?.status === 'LEAN';
  const valueCleanEnough = entryValue.label !== 'BAD' && entryValue.score >= 48;
  const qualityCleanEnough = tradeQuality.label === 'DECENT' || tradeQuality.label === 'STRONG';
  const sideIsPlayable = side !== 'NONE' && signalStableEnough && planMatureEnough && qualityCleanEnough && valueCleanEnough && decision.settlement.risk !== 'High' && decision.settlement.risk !== 'Extreme' && flipRisk.level !== 'High';
  const projectedPick: PreCommitPreview['projectedPick'] = sideIsPlayable ? side : 'NO TRADE';
  const wouldLockNow: PreCommitPreview['wouldLockNow'] = secondsToLock <= 0 && sideIsPlayable ? 'YES' : 'NO';
  const tone: Tone = projectedPick === 'NO TRADE' ? (strength >= 70 ? 'warn' : 'neutral') : strength >= 76 ? 'good' : 'blue';
  const message = projectedPick === 'NO TRADE'
    ? `Stable preview is not showing a playable side yet. It needs a mature plan, 2+ confirmations, stable signal, clean quality/value, and low flip risk before it previews OVER or UNDER.`
    : `Stable preview is holding ${projectedPick} as the current lock candidate. This should change less often than the raw live read, but it is still not locked until the commitment time.`;
  const changes = projectedPick === 'NO TRADE'
    ? ['2+ confirmations form', 'Trade Quality is DECENT/STRONG', 'Entry Value is not BAD', 'Flip risk stays Low or Medium']
    : [`Sustained signal flips away from ${projectedPick}`, 'Entry Value drops to BAD', 'Trade Quality weakens below DECENT', 'Late flip risk jumps to High'];
  return { projectedPick, wouldLockNow, strength, timeUntilLock, message, changes, tone };
}
  ...
  return (
    <Panel title="Entry Value Engine">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
        ...
        <Metric label="Market Ask" value={value.askCents === null ? 'Unavailable' : `${value.askCents}¢`} detail={value.grossProfitCents === null ? 'Model-only value' : `Upside ${value.grossProfitCents}¢ • risk ${value.riskCents}¢`} help="A cheap entry can be valuable if Edge15 has real edge. When Kalshi ask is unavailable, Edge15 falls back to model-only value and does not pretend it knows the real payout." tone={value.askCents === null ? 'warn' : value.askCents <= 70 ? 'good' : value.askCents <= 86 ? 'warn' : 'bad'} />
  ...
  return (
    <Panel title="Early Entry Lab">
      <div className="grid gap-3 sm:grid-cols-4">
        <Metric label="Current Phase" value={currentPhase} detail={countdown.display} help="Genesis-027 live mode locks at 4:00 remaining while value evidence keeps building in the labs." tone="blue" />
        <Metric label="Clean Early" value={cleanEarlyArmed ? 'ARMED' : 'WAIT'} detail={entryValue.label} help="ARMED means Edge15 sees enough value to consider an earlier clean setup in the lab, but live lock remains 4:00." tone={cleanEarlyArmed ? 'good' : 'warn'} />
      ...
      <div className="mt-3 rounded-2xl border border-edge-blue/30 bg-edge-blue/10 p-3 text-xs leading-5 text-edge-blue">
        Genesis-027 uses the 4:00 live commitment while continuing to observe early-entry value in the labs. It still uses NO TRADE when value, quality, or stability are not clean enough.
      </div>
    </Panel>
  );
}
...
function buildEntryValue(decision: Decision, activeSignal: SignalPlan | null, countdown: Countdown, snapshot: MarketSnapshot, tradeQuality: TradeQuality, autoTightening: AutoTighteningProfile, flipRisk: FlipRisk, timingRecords: CommitTimingRecord[], profileRecords: StrategyProfileRecord[]): EntryValue {
  ...
  const expensivePenalty = askCents === null ? 3 : askCents >= 92 ? 22 : askCents >= 86 ? 12 : askCents >= 78 ? 5 : 0;
  const edgeScore = edgePct === null ? 0 : edgePct * 1.45;
  ...
  const askRead = askCents === null ? 'Kalshi ask is unavailable. Entry Value is in model-only mode, so Edge15 can estimate direction quality but cannot verify true payout edge.' : `${side} ask is ${askCents}¢ with estimated win probability near ${estimatedWinProbability}%, giving estimated edge ${edgePct! >= 0 ? '+' : ''}${edgePct}%.`;
  ...
  const message = label === 'BAD'
    ...
    : label === 'FAIR'
      ...
      : `${askRead} This is the type of price-versus-probability setup Genesis-027 is designed to track.${lateRead}`;
...
function AdaptiveCommitLabPanel({ records, currentRead, countdown }: { records: AdaptiveCommitRecord[]; currentRead: AdaptiveCommitRead; countdown: Countdown }) {
  ...
  return (
    <Panel title="Adaptive Commit Lab">
      ...
      <div className="mt-3 rounded-2xl border border-edge-blue/30 bg-edge-blue/10 p-3 text-xs leading-5 text-edge-blue">
        Genesis-027 keeps adaptive commit testing hidden by default. It still compares the 4:00 live lock against a shadow engine that can commit when the setup becomes ready.
      </div>
    </Panel>
  );
}
...
function buildAdaptiveCommitRead({ decision, activeSignal, countdown, snapshot, entryValue, tradeQuality, autoTightening, flipRisk }: { decision: Decision; activeSignal: SignalPlan | null; countdown: Countdown; snapshot: MarketSnapshot; entryValue: EntryValue; tradeQuality: TradeQuality; autoTightening: AutoTighteningProfile; flipRisk: FlipRisk }): AdaptiveCommitRead {
  ...
  if (remainingMs > 8 * 60 * 1000) {
    ...
    if (earlyClean) {
      return { status: 'Committed', direction: side, shouldCapture: true, mode: 'EARLY_CLEAN', score, tone: 'good', reason: `Adaptive lab would commit ${side} early because Trade Quality is STRONG and Entry Value is ${entryValue.label} before the fixed 4:00 commit point.` };
    }
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <div>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs leading-5 text-edge-muted">Genesis-027 keeps timing value available behind the Timing Lab toggle. A perfect late read can still be a bad entry if the ask is too expensive.</div>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-edge-amber/30 bg-edge-amber/10 p-3 text-xs leading-5 text-edge-amber">
            Early checkpoints can pay better but need cleaner signal. Late checkpoints may predict better but often have weak payout. Genesis-027 starts measuring both sides of that tradeoff.
          </div>
        </div>
      </div>
    </Panel>
...
function PerformanceTrackerPanel({ records }: { records: CommitmentAccuracyRecord[] }) {
  const [allTime] = buildPerformanceWindows(records);
  const tone = allTime.winRate === null ? 'neutral' : allTime.winRate >= 85 ? 'good' : allTime.winRate >= 70 ? 'warn' : 'bad';
  const totalTracked = allTime.wins + allTime.losses + allTime.noTrades;
  return (
    <Panel title="Long-term performance tracker">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
        <Metric label="All-time W/L" value={`${allTime.wins}-${allTime.losses}`} detail={allTime.winRate === null ? 'No resolved trades yet' : `${allTime.winRate}% win rate`} help="This is the long tracker that matters most: automatic wins and losses from committed OVER/UNDER plans. NO TRADE is separate and does not count as a loss." tone={tone} />
        <Metric label="No Trades" value={`${allTime.noTrades}`} detail="Safety skips" help="NO TRADE means Edge15 refused a low-quality setup. It protects the win rate but should be tracked separately from scored trades." tone="blue" />
        <Metric label="Scored Trades" value={`${allTime.resolved}`} detail="Wins + losses" help="Only committed OVER/UNDER plans count here. This is the sample size for the all-time win rate." tone="neutral" />
        <Metric label="Stored Total" value={`${totalTracked}`} detail="All-time local records" help="Records are stored in this browser. Export/backup before clearing cache or switching devices." tone="neutral" />
      </div>
      <div className="mt-3 rounded-2xl border border-edge-amber/30 bg-edge-amber/10 p-3 text-xs leading-5 text-edge-amber">
        Time-window rows were removed. Genesis-027 focuses on long-term all-time W/L because that is the cleaner scorecard while we dial the model.
      </div>
    </Panel>
  );
}
...
function CommitmentAccuracyPanel({ records, activeSignal }: { records: CommitmentAccuracyRecord[]; activeSignal: SignalPlan | null }) {
  ...
  return (
    <Panel title="Commitment accuracy">
      <div className="grid gap-3 sm:grid-cols-3">
        <Metric label="Last 10" value={last.length ? `${correct}/${resolved.length}` : '—'} detail="Resolved commitments" help="This grades Edge15's locked 4:00 commitment prediction, not later live recommendations." tone={accuracy === null ? 'neutral' : accuracy >= 60 ? 'good' : accuracy >= 50 ? 'warn' : 'bad'} />
...
function PreCommitPreviewCard({ preview }: { preview: PreCommitPreview }) {
  const toneClass = preview.tone === 'good'
    ...
    : preview.tone === 'bad'
      ...
      : preview.tone === 'warn'
        ...
        : 'border-edge-blue/40 bg-edge-blue/10 text-edge-blue';
  return (
    <div className={`mt-3 rounded-2xl border p-3 ${toneClass}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="text-[11px] font-black uppercase tracking-[0.18em] opacity-80">Stable Lock Candidate</div>
          <div className="mt-1 text-2xl font-black leading-tight">{highlightText(preview.projectedPick)}</div>
        </div>
        <div className="text-right text-xs leading-5">
          <div><span className="font-black">Lock:</span> {preview.timeUntilLock}</div>
          <div><span className="font-black">Lock-ready:</span> {preview.wouldLockNow}</div>
          <div><span className="font-black">Strength:</span> {preview.strength}/100</div>
        </div>
      </div>
      <div className="mt-2 text-xs leading-5 text-slate-200">{highlightText(preview.message)}</div>
      <div className="mt-2 grid gap-1 text-[11px] leading-4 text-slate-300 sm:grid-cols-2">
        {preview.changes.slice(0, 4).map((change) => <div key={change} className="rounded-lg border border-edge-line bg-black/20 px-2 py-1">{highlightText(change)}</div>)}
      </div>
    </div>
  );
}
...
function PositionExitPanel({ read }: { read: PositionExitRead }) {
  return (
    <div className={`mt-4 rounded-3xl border p-4 ${read.tone === 'good' ? 'border-edge-green/30 bg-edge-green/10' : read.tone === 'warn' ? 'border-edge-amber/30 bg-edge-amber/10' : read.tone === 'bad' ? 'border-edge-red/30 bg-edge-red/10' : 'border-edge-blue/30 bg-edge-blue/10'}`}>
      <div className="mb-3 text-xs uppercase tracking-[0.22em] text-edge-muted">Position Exit Engine</div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Exit status" value={read.status} detail={read.status === 'CASH OUT SIGNAL' ? 'Thesis breaking' : 'Position management'} help="CASH OUT SIGNAL is intentionally hard to trigger. It requires multiple signs that the original position thesis is failing." tone={read.tone} />
        <Metric label="Hold Quality" value={read.holdQuality} detail={`${read.score}/100`} help="Rates whether the locked position still has support from the live model, strike location, momentum, settlement risk, and value." tone={read.tone} />
        <Metric label="Profit Protection" value={read.profitProtection} detail="If available" help="A warning to protect gains or avoid giving back value when the position is still alive but weakening." tone={read.profitProtection === 'ACTIVE' ? 'warn' : 'neutral'} />
        <Metric label="Salvage" value={read.salvageWarning} detail="Avoid full-loss risk" help="A salvage warning means the trade is weakening enough that exiting early may be better than hoping for a full reversal." tone={read.salvageWarning === 'ACTIVE' ? 'bad' : 'neutral'} />
      </div>
      <div className="mt-3 grid gap-2 text-xs leading-5 text-slate-200 sm:grid-cols-2">
        {read.reasons.slice(0, 4).map((reason) => <div key={reason} className="rounded-xl border border-edge-line bg-black/20 px-3 py-2">{highlightText(reason)}</div>)}
      </div>
    </div>
  );
}

function buildPositionExitRead(position: LockedPosition, assessment: NonNullable<ReturnType<typeof assessPosition>>, decision: Decision, countdown: Countdown, tradeQuality: TradeQuality, entryValue: EntryValue, flipRisk: FlipRisk): PositionExitRead {
  const reasons: string[] = [];
  let penalty = 0;
  const oppositeSignal = decision.direction !== 'NONE' && decision.direction !== position.side;
  const wrongSideOfStrike = position.side === 'OVER'
    ? decision.distanceToReference !== null && decision.distanceToReference < 0
    : position.side === 'UNDER'
      ? decision.distanceToReference !== null && decision.distanceToReference > 0
      : false;
  const movedAgainstEntry = assessment.distanceSinceEntry !== null
    ? (position.side === 'OVER' ? assessment.distanceSinceEntry < -20 : assessment.distanceSinceEntry > 20)
    : false;
  const favorableFromEntry = assessment.distanceSinceEntry !== null
    ? (position.side === 'OVER' ? assessment.distanceSinceEntry > 25 : assessment.distanceSinceEntry < -25)
    : false;

  if (oppositeSignal && decision.confidence >= 72) {
    penalty += 30;
    reasons.push(`Live model now favors ${decision.direction} against your ${position.side}.`);
  } else if (oppositeSignal) {
    penalty += 16;
    reasons.push(`Live model is leaning against your ${position.side}, but not strongly enough for a forced exit signal.`);
  }
  if (wrongSideOfStrike) {
    penalty += 24;
    reasons.push(`${position.side} is now on the wrong side of the reference strike.`);
  }
  if (movedAgainstEntry) {
    penalty += 13;
    reasons.push('BTC has moved against your entry since the position was locked.');
  }
  if (decision.settlement.risk === 'Extreme') {
    penalty += 25;
    reasons.push('Settlement risk is Extreme.');
  } else if (decision.settlement.risk === 'High') {
    penalty += 17;
    reasons.push('Settlement risk is High.');
  }
  if (flipRisk.level === 'High') {
    penalty += 17;
    reasons.push('Late flip risk is High.');
  } else if (flipRisk.level === 'Medium') {
    penalty += 7;
    reasons.push('Flip risk is Medium.');
  }
  if (tradeQuality.label === 'AVOID') {
    penalty += 18;
    reasons.push('Trade Quality has collapsed to AVOID.');
  } else if (tradeQuality.label === 'WEAK') {
    penalty += 9;
    reasons.push('Trade Quality has weakened.');
  }
  if (entryValue.label === 'BAD') {
    penalty += 10;
    reasons.push('Entry Value is now BAD, so payout/value no longer supports chasing.');
  }
  if (countdown.remainingMs <= 180000 && wrongSideOfStrike) {
    penalty += 18;
    reasons.push('Final 3 minutes and position is on the wrong side of strike.');
  }
  if (!reasons.length) reasons.push('No major thesis-break signal yet. Continue monitoring normally.');

  const score = Math.max(0, Math.min(100, 100 - penalty));
  const holdQuality: PositionExitRead['holdQuality'] = score >= 78 ? 'STRONG' : score >= 58 ? 'OK' : score >= 34 ? 'WEAK' : 'BROKEN';
  const status: PositionExitRead['status'] = penalty >= 78 ? 'CASH OUT SIGNAL' : penalty >= 56 ? 'DANGER' : penalty >= 30 ? 'WATCH CLOSELY' : 'HOLD';
  const tone: Tone = status === 'HOLD' ? 'good' : status === 'WATCH CLOSELY' ? 'warn' : 'bad';
  const profitProtection = favorableFromEntry && (oppositeSignal || tradeQuality.label === 'WEAK' || flipRisk.level === 'High') ? 'ACTIVE' : 'QUIET';
  const salvageWarning = (status === 'DANGER' || status === 'CASH OUT SIGNAL') && (wrongSideOfStrike || movedAgainstEntry || oppositeSignal) ? 'ACTIVE' : 'QUIET';
  return { status, holdQuality, score, profitProtection, salvageWarning, reasons, tone };
}
...
function buildValueGate(direction: SignalDirection, countdown: Countdown, snapshot: MarketSnapshot): EntryGate {
  ...
  if (askCents === null) {
    const finalThree = countdown.remainingMs <= 180000;
    return {
      label: 'Payout value',
      passed: !finalThree,
      detail: finalThree ? 'Kalshi ask is unavailable in the final 3 minutes, so Edge15 blocks fresh entries because payout value cannot be verified.' : 'Kalshi ask is unavailable. Edge15 is using model-only value and will not treat this as confirmed payout edge.',
      severity: finalThree ? 'block' : 'warn',
    };
  }
...
function buildEntryGates(decision: Decision, activeSignal: SignalPlan | null, countdown: Countdown, qualityFilter: QualityFilter, snapshot: MarketSnapshot, autoTightening: AutoTighteningProfile, flipRisk: FlipRisk, tradeQuality: TradeQuality): EntryGate[] {
  ...
  return [
    ...
    },
    {
      ...
      detail: `${decision.settlement.risk}. Clean ENTER needs Low settlement risk in Genesis-027. ${decision.settlement.message}`,
...
function buildLateEntryWarning(decision: Decision, countdown: Countdown) {
  ...
  if (countdown.remainingMs <= 180000) return `Only ${countdown.display} remains. Genesis-027 blocks fresh late entries because the last 3 minutes are too jumpy and the payout is often too small.`;
...
function optionalPanelLabel(key: OptionalPanelKey) {
  const labels: Record<OptionalPanelKey, string> = {
    trackerStatus: 'Tracker Status',
    commitmentAccuracy: 'Commit Accuracy',
    backupCompare: 'Backup',
    entryValue: 'Entry Value',
    commitTimingLab: 'Timing Lab',
    adaptiveCommitLab: 'Adaptive Lab',
    strategyProfileLab: 'Strategy Lab',
    focusedDashboard: 'Decision Details',
