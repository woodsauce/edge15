...
const COMMIT_AFTER_MS = 3 * 60 * 1000;
...
export function updateSignalPlan({ previous, decision, countdown, now }: SignalPlanInput): SignalPlan {
  ...
  // Once Edge15 commits after minute 3, the prediction side stays fixed for the
...
function createPlan(contractKey: string, direction: Decision['direction'], rawStatus: SignalStatus, decision: Decision, countdown: SignalPlanInput['countdown'], nowIso: string, tone: Tone): SignalPlan {
  ...
  return withNarrative({
    ...
    committedAt: null,
    commitmentReason: 'Scout Mode: Edge15 has not reached the minute-3 commitment point yet.',
  }, decision, countdown);
}
...
function maybeCommit(plan: SignalPlan, decision: Decision, countdown: SignalPlanInput['countdown'], nowIso: string): SignalPlan {
  ...
  if (elapsedMs < COMMIT_AFTER_MS) {
    return withNarrative({
      ...
      committedDirection: 'NONE',
      committedAt: null,
      commitmentReason: `Scout Mode until minute 3. Edge15 is gathering evidence and will not lock the contract prediction yet.`,
    }, decision, countdown);
  }
  ...
  if (plan.direction === 'NONE' || decision.opportunity < 50 || decision.confidence < 52 || decision.settlement.risk === 'Extreme') {
    return withNarrative({
      ...
      committedDirection: 'NONE',
      committedAt: nowIso,
      commitmentReason: `Minute-3 commitment check did not find a clean edge. Edge15 is intentionally sitting this contract out instead of flipping later.`,
    }, decision, countdown);
  }
  ...
  return withNarrative({
    ...
    committedDirection,
    committedAt: nowIso,
    commitmentReason: `At the minute-3 commitment check, Edge15 locked ${committedDirection}. The prediction side will not flip for the rest of this 15-minute contract; only the management status can change.`,
  }, decision, countdown);
}
...
function withNarrative(plan: SignalPlan, decision: Decision, countdown: SignalPlanInput['countdown']): SignalPlan {
  ...
  if (plan.commitmentStatus === 'SCOUTING') planText = `Scout Mode: Edge15 is watching the opening 3 minutes. No final contract prediction is locked yet.`;
  if (plan.commitmentStatus === 'NO TRADE') planText = `No Trade is committed for this contract. Edge15 did not see enough edge at the minute-3 check.`;
  ...
  if (plan.status === 'NO PLAN' && plan.commitmentStatus === 'SCOUTING') planText = 'No active plan. Edge15 is waiting for a directional idea to form before the minute-3 commitment check.';
...
function buildNextStep(plan: SignalPlan, decision: Decision, display: string) {
  if (plan.commitmentStatus === 'SCOUTING') return `Scout Mode. At about 12:00 remaining, Edge15 will commit to OVER, commit to UNDER, or mark this contract NO TRADE.`;
  if (plan.commitmentStatus === 'NO TRADE') return `No Trade was committed at minute 3. Edge15 will continue showing risk context, but it will not chase a late prediction for this contract.`;
