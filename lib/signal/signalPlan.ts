...
const COMMIT_AFTER_MS = 11 * 60 * 1000;
...
export function updateSignalPlan({ previous, decision, countdown, now }: SignalPlanInput): SignalPlan {
  ...
  // Once Edge15 commits at the 4:00-left test point, the prediction side stays fixed for the
...
function createPlan(contractKey: string, direction: Decision['direction'], rawStatus: SignalStatus, decision: Decision, countdown: SignalPlanInput['countdown'], nowIso: string, tone: Tone): SignalPlan {
  ...
  return withNarrative({
    ...
    committedAt: null,
    commitmentReason: 'Scout Mode: Edge15 has not reached the 4:00-left commitment point yet.',
...
function maybeCommit(plan: SignalPlan, decision: Decision, countdown: SignalPlanInput['countdown'], nowIso: string): SignalPlan {
  ...
  if (elapsedMs < COMMIT_AFTER_MS) {
    return withNarrative({
      ...
      committedDirection: 'NONE',
      committedAt: null,
      commitmentReason: `Scout Mode until 4:00 left. Edge15 is gathering evidence and will not lock the contract prediction yet.`,
    }, decision, countdown);
  }
  ...
  if (plan.direction === 'NONE' || commitmentTooWeak || decision.settlement.risk === 'High' || decision.settlement.risk === 'Extreme' || wrongSideAtCommitment) {
    return withNarrative({
      ...
      committedDirection: 'NONE',
      committedAt: nowIso,
      commitmentReason: `4:00-left commitment test did not find a clean, protected edge. Edge15 is intentionally sitting this contract out instead of forcing a side.`,
  ...
  return withNarrative({
    ...
    committedDirection,
    committedAt: nowIso,
    commitmentReason: `At the 4:00-left commitment test, Edge15 locked ${committedDirection}. The prediction side will not flip for the rest of this 15-minute contract; only the management status can change.`,
    committedEntryScore: decision.entryScore,
    committedConfidence: decision.confidence,
    committedTradeGrade: decision.tradeGrade,
    committedSettlementRisk: decision.settlement.risk,
...
function withNarrative(plan: SignalPlan, decision: Decision, countdown: SignalPlanInput['countdown']): SignalPlan {
  ...
  if (plan.commitmentStatus === 'NO TRADE') planText = `No Trade is committed for this contract. Edge15 did not see enough edge at the 4:00-left test.`;
  ...
  if (plan.status === 'NO PLAN' && plan.commitmentStatus === 'SCOUTING') planText = 'No active plan. Edge15 is waiting for a directional idea to form before the 4:00-left commitment test.';
...
function buildNextStep(plan: SignalPlan, decision: Decision, display: string) {
  if (plan.commitmentStatus === 'SCOUTING') return `Scout Mode. At about 4:00 remaining, Edge15 will commit to OVER, commit to UNDER, or mark this contract NO TRADE.`;
  if (plan.commitmentStatus === 'NO TRADE') return `No Trade was committed at the 4:00-left test. Edge15 will continue showing risk context, but it will not chase a late prediction for this contract.`;
